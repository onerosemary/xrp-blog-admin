...
export default {
  ...
  methods: {
    ...
    initWebSocket() {
      let _this = this
      if (window.WebSocket) {
        // ws://medicalbeauty-back.bx-cy.com/webSocket?fid=当前登录用户id&toid=接收信息的客户id&eid=0&source=1
        // const uri = `ws://medicalbeauty-back.bx-cy.com/webSocket?fid=${this.$store.getters.userId}&toid=${this.toId}&eid=0&source=1`
        const uri = `ws://120.25.247.94:8088/webSocket?fid=${this.$store.getters.userId}&toid=${this.toId}&eid=0&source=1`
